...
const clicmd_t cmd_table[] = {
    CLI_COMMAND_DEF("defaults", "reset to defaults and reboot", NULL,
                    cli_cmd_defaults),
    CLI_COMMAND_DEF("dump", "Dump configuration", NULL, cli_cmd_dump),
    CLI_COMMAND_DEF("get", "get variable value", "[cmd_name]", cli_cmd_get),
    CLI_COMMAND_DEF("help", "display command help", "[search string]",
                    cli_cmd_help),
    CLI_COMMAND_DEF("log_enable", "enable the logging output", NULL,
                    cli_cmd_log_enable),
    CLI_COMMAND_DEF("reboot", "reboot without saving", NULL, cli_cmd_reboot),
    CLI_COMMAND_DEF("read", "read last recording", NULL, cli_cmd_read),
    CLI_COMMAND_DEF("save", "save configuration", NULL, cli_cmd_save),
    CLI_COMMAND_DEF("set", "change setting", "[<cmd_name>=<value>]",
                    cli_cmd_set),
...
static void cli_cmd_help(const char *cmd_name, char *args) {
  ...
  for (uint32_t i = 0; i < ARRAYLEN(cmd_table); i++) {
    ...
    if (is_empty(args)) {
      ...
      if (strstr(cmd_table[i].name, args) ||
          strstr(cmd_table[i].description, args)) {
        print_entry = true;
      }
    }
...
static void cli_cmd_set(const char *cmd_name, char *args) {
  ...
  if (len == 0 || (len == 1 && args[0] == '*')) {
    ...
    for (uint32_t i = 0; i < value_table_entry_count; i++) {
      ...
      cli_printf("%s = ", value_table[i].name);
      // when len is 1 (when * is passed as argument), it will print min/max
      // values as well, for gui
    ...
    case MODE_BITSET: {
      ...
      for (uint32_t tableValueIndex = 0;
           tableValueIndex < tableEntry->value_count && !matched;
           tableValueIndex++) {
...
static void cli_cmd_read(const char *cmd_name, char *args) {
  ...
  if (count == 0) {
    cli_printf("No Recording found");
    return;
  }

  for (int i = 1; i < count + 1; i++) {
    recorder_element_t element = {0};
    bool status = recorder_get_element(&recorder, i, &element);
    if (status == true) {
      cli_printf("%d, %3s, %d\n", element.ts, recorder_lookup[element.type],
                 element.data);
    }
  }
}
