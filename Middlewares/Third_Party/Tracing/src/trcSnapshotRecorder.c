 ...
 ******************************************************************************/
void prvTraceInitTimestamps(void) { init_hwtc_count = TRC_HWTC_COUNT; }
...
void vTraceStoreISREnd(int pendingISR) {
  ...
  trcCRITICAL_SECTION_BEGIN();
  isPendingContextSwitch |=
      pendingISR; /* Is there a pending context switch right now? If so, we will
                     not create an event since we will get an event when that
