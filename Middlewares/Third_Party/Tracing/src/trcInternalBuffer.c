...
#include "../../Tracing/inc/trcRecorder.h"
    ...
    (TRC_STREAM_PORT_USE_INTERNAL_BUFFER == 1)
...
PageType PageInfo
    [TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT] TRC_CFG_RECORDER_DATA_ATTRIBUTE;
...
void prvPagedEventBufferInit(char *buffer) {
  TotalBytesRemaining_LowWaterMark = (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) *
                                     (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE);
  TotalBytesRemaining = (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_COUNT) *
                        (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE);
